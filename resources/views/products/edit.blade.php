            ...
            <div class="shadow sm:overflow-hidden sm:rounded-md">
                <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6">
                            ...
                            <x-trix-input
                                id="body_html"
                                name="body_html"
                                :value="old('body_html', $product->body_html)"
                                class="mt-1"
                            />
                            @error('body_html')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        ...
                        <p class="text-sm text-gray-500 mb-4">Manage product images. Upload new images or remove existing ones.</p>

                        <!-- Existing Images -->
                        @if($product->images->count() > 0)
                            <div class="mb-6">
                                <h4 class="text-sm font-medium text-gray-700 mb-3">Current Images ({{ $product->images->count() }})</h4>
                                <div class="grid grid-cols-4 gap-4">
                                    @foreach ($product->images as $image)
                                        <div class="relative group">
                                            <img src="{{ $image->url }}" alt="Product Image" class="h-32 w-full object-cover rounded-md border border-gray-200">
                                            <form action="{{ route('products.images.destroy', [$product, $image]) }}" method="POST" class="absolute top-1 right-1">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Are you sure you want to delete this image?')" class="bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600" title="Delete Image">
                        ...
                        <div class="mt-4">
                            ...
                            <form action="{{ route('products.images.store', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                @csrf
                                <div class="flex justify-center rounded-md border-2 border-dashed border-gray-300 px-6 pt-5 pb-6">
                                    <div class="space-y-1 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600">
                                            <label for="image-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:text-indigo-500">
